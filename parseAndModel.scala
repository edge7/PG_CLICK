...
val kModel = sc.objectFile[org.apache.spark.mllib.clustering.KMeansModel]("kMeans21.model").first()
...
val defaultK = kModel.predict(Vectors.dense(0))  
...
val testMSE = 
for ( numTrees <- (20 to 500 by 50);
	  maxDepth <- (4  to 30  by 5)   )
yield {
	...
	(numTrees, maxDepth, labelsAndPredictions.map{ case(v, p) => math.pow((v - p), 2) }.mean() )
